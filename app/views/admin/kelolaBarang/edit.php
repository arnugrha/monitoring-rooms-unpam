    ...
    <section class="p-4 md:p-6 lg:p-8 space-y-6 max-w-[800px] mx-auto">
        ...
        <div class="mb-8 text-center sm:text-left">
            <div class="flex items-center gap-2 text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2 justify-center sm:justify-start">
                ...
                <span class="text-primary">Edit Barang</span>
            </div>
            <h1 class="text-2xl md:text-3xl font-extrabold text-on-surface font-headline leading-tight">
                Edit Barang: <?= $data['barang']['nama_barang']; ?>
            </h1>
            <p class="text-slate-500 mt-2 text-sm md:text-base leading-relaxed">
                Perbarui detail informasi barang untuk didaftarkan ke dalam sistem pemantauan pusat.
            </p>
        </div>

        <!-- Form Container -->
        <div class="bg-white rounded-lg p-6 md:p-8 shadow-sm border border-slate-100">
            <form action="<?= BASEURL; ?>kelolaBarang/prosesUbah" method="post" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div class="space-y-2">
                        ...
                        <input class="w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all outline-none text-sm text-on-surface placeholder:text-slate-300" placeholder="Masukan Nama Barang" name="nama_barang" type="text" value="<?= $data['barang']['nama_barang']; ?>"/>
                    </div>
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select name="id_kategori" required
                                ...
                                <?php foreach ($data['kategori'] as $kat) : ?>
                                    <option value="<?= $kat['id_kategori']; ?>" <?= ($data['barang']['id_kategori'] == $kat['id_kategori']) ? 'selected' : ''; ?>>
                                        <?= $kat['nama_kategori']; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                ...
                <div class="pt-6 flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-slate-50">
                    <a href="<?= BASEURL; ?>kelolaBarang" class="w-full sm:w-auto px-6 h-11 text-slate-500 font-bold text-sm hover:bg-slate-50 rounded-lg transition-all" type="button">
                        Batal
                    </a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mt-8">
            <div class="bg-blue-50/50 p-5 rounded-lg flex flex-col gap-3 border border-blue-100/50">
                ...
                <p class="text-[10px] md:text-xs text-slate-500 leading-relaxed">
                    Pastikan informasi barang sesuai sebelum melakukan pembaruan.
                </p>
            </div>
