...
<section class="p-4 md:p-6 lg:p-8 space-y-6 max-w-[800px] mx-auto">
    ...
    <div class="mb-8 text-center sm:text-left">
        <div class="flex items-center gap-2 text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2 justify-center sm:justify-start">
            ...
            <span class="text-primary">Edit Container</span>
        </div>
        ...
        <p class="text-slate-500 mt-2 text-sm md:text-base leading-relaxed text-balance">
            Perbarui informasi unit penyimpanan atau pindahkan lokasinya ke ruangan lain.
        </p>
    </div>

    <!-- Form Container -->
    <div class="bg-white rounded-lg p-6 md:p-8 shadow-sm border border-slate-100">
        <form action="<?= BASEURL; ?>index.php?url=container/prosesEdit" method="post" class="space-y-6">
            <!-- Hidden ID -->
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative custom-select-container" id="room-select-container">
                        ...
                        <input type="hidden" name="kode_ruangan" id="kode_ruangan" value="<?= $data['container']['kode_ruangan']; ?>" required>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select name="id_barang" required
                            ...
                            <option value="" disabled>Pilih Jenis/Nama Container</option>
                            <?php foreach ($data['barang'] as $b) : ?>
                                <option value="<?= $b['id_barang']; ?>" <?= ($b['id_barang'] == $data['container']['id_barang']) ? 'selected' : ''; ?>>
                                    <?= $b['nama_barang']; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
            ...
            <div class="pt-6 flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-slate-50">
                ...
                <button 
                    class="w-full sm:w-auto px-10 h-11 bg-primary text-white font-bold rounded-lg shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm" 
                    type="submit"
                >
    ...
    <!-- QR Data Section (Only if exists) -->
    <?php if($data['container']['qr_data']) : ?>
    <div class="bg-white rounded-lg p-6 md:p-8 shadow-sm border border-slate-100 mt-6">
        <div class="flex flex-col md:flex-row items-center gap-6">
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 shadow-inner">
                <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=<?= urlencode($data['container']['qr_data']); ?>" alt="QR Code" class="w-32 h-32 md:w-40 md:h-40">
            </div>
            <div class="space-y-4 flex-1">
                <div>
                    <h3 class="text-sm font-bold text-slate-700 uppercase tracking-wider">Public Access URL</h3>
                    <div class="mt-1 flex items-center gap-2">
                        <code class="flex-1 block p-2 bg-slate-50 border border-slate-200 rounded text-xs text-blue-600 truncate"><?= $data['container']['qr_data']; ?></code>
                        <button type="button" onclick="window.open('<?= $data['container']['qr_data']; ?>', '_blank')" class="p-2 text-primary hover:bg-primary/5 rounded-lg transition-colors" title="Buka URL">
                ...
                <div class="pt-2">
                    <p class="text-xs text-slate-400 leading-relaxed italic">
                        QR Code ini mengarah ke halaman publik container. Anda dapat mencetaknya dan menempelkannya pada fisik container.
                    </p>
                </div>
                <a href="https://api.qrserver.com/v1/create-qr-code/?size=1000x1000&data=<?= urlencode($data['container']['qr_data']); ?>" download="QR_Container_<?= $data['container']['id_container']; ?>.png" target="_blank" class="inline-flex items-center gap-2 text-xs font-bold text-primary hover:underline">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Initialize search input with current selection
            const currentVal = hiddenInput.value;
            if (currentVal) {
                const selectedOpt = Array.from(options).find(opt => opt.dataset.value === currentVal);
                if (selectedOpt) {
                    const text = selectedOpt.querySelector('span:first-child').textContent;
                    searchInput.value = text + ' (' + currentVal + ')';
                }
            }
